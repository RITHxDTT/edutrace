...
export default function SignUpForm({ classrooms }: ClassroomProps) {
  ...
  const selectedClassroom = classrooms.find(
    (c) => c.classroomId === values.classroomId,
  );
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center gap-2">
        {STEPS.map((s, i) => (
          <div key={s.id} className="flex items-center gap-2">
            ...
            {i < STEPS.length - 1 && (
              <div
                className={`h-px w-8 transition-all duration-300 ${step > s.id ? "bg-green-400" : "bg-gray-200"}`}
              />
            )}
          </div>
        ))}
      ...
      <form onSubmit={handleSubmit(onSubmit)} noValidate>
        ...
        {/* Navigation buttons */}
        <div
          className={`flex mt-6 gap-3 ${step > 1 ? "justify-between" : "justify-end"}`}
        >
          {step > 1 && (
            <PrimaryButton
              onClick={handleBack}
