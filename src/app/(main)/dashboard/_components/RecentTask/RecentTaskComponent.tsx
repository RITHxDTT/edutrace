...
import { DashboardRecentTask } from '@/types/dashboard';

export default function RecentTaskComponent({ tasks }: { tasks: DashboardRecentTask[] }) {
    return (
        <div className="bg-white text-black rounded-3xl p-6 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] max-w-4xl mx-auto w-full border border-gray-100/50">
            ...
            >
                ...
                <TableBody>
                    {(tasks || []).map((task) => (
                        <TableRow key={task.assessmentId} className="border-b border-slate-100/60 last:border-0 hover:bg-slate-50/30 transition-colors">
