
import { Notification } from "./notification.types";

export const mockNotifications: Notification[] = [
  {
    notificationId: crypto.randomUUID(),
    title: "New Assignment Published",
    content: "UI Design Systems - Module 4",
    type: "ASSIGNMENT",
    isRead: false,
    createdAt: "2026-05-24T13:18:36.532Z",
  },
  {
    notificationId: crypto.randomUUID(),
    title: "Instructor Feedback",
    content: "Your Data Structure submission was reviewed.",
    type: "FEEDBACK",
    isRead: true,
    createdAt: "2026-05-24T12:18:36.532Z",
  },
  {
    notificationId: crypto.randomUUID(),
    title: "Hello World",
    content: "Your submission has been reviewed.",
    type: "FEEDBACK",
    isRead: true,
    createdAt: "2026-05-22T12:18:36.532Z",
  },
  {
    notificationId: crypto.randomUUID(),
    title: "Class Schedule Changed",
    content: "Advanced UX Research is moved to SR.",
    type: "FEEDBACK",
    isRead: false,
    createdAt: "2026-05-21T09:00:00.000Z",
  },
  {
    notificationId: crypto.randomUUID(),
    title: "Homework",
    content: "Doing on homework korean.",
    type: "FILE_UPLOAD",
    isRead: true,
    createdAt: "2026-05-20T15:30:22.000Z",
  },
  {
    notificationId: crypto.randomUUID(),
    title: "New Reply in Forum",
    content: "Alex commented on your Next.js question.",
    type: "ASSIGNMENT",
    isRead: false,
    createdAt: "2026-05-20T11:15:00.000Z",
  },
];